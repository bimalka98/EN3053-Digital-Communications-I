...
QMSK = zeros(num_of_Xks, resolution);
% generating quaternary MSK waveform
for n = 0:(num_of_Xks-1)
    t = linspace(n*T, (n+1)*T, resolution);
    phase = (pi/2)*sum(X_ks(1:n)) + (pi/2)*X_ks(n+1)*(t/T -n);
    QMSK(n+1,:) = sqrt(2/T)*cos(2*pi*t/T + phase);
end
QMSK = reshape(QMSK',1,[]); % pre-processing for plotting
plot(linspace(0,num_of_Xks*T, length(QMSK)), QMSK, 'LineWidth',3);
